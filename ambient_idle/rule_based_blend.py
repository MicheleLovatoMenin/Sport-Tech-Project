...
def create_sequential_strips(armature, state_sequence, shot_anim_name, p_traj, shot_blender_start):
    print("🎬 Creazione Timeline (Metodo Scala - 100% NO T-POSE)...")
    ...
    # Pulizia totale delle tracce precedenti
    ...
    # Contatore per creare sempre nuove tracce sopra le precedenti
    track_counter = 1
    ...
    for state, group in grouped_states:
        ...
        # === 2. ESTENSIONE STRIP PRECEDENTE (BASE SOLIDA) ===
        # La strip sotto deve durare abbastanza da coprire l'entrata di quella nuova.
        # Estendiamo la fine della vecchia strip.
        if prev_strip:
            overlap_end = strip_start + BLEND_FRAMES
            # Se la vecchia strip finiva prima, la allunghiamo.
            # Se finiva dopo, la accorciamo (taglio netto sotto il blend).
            if overlap_end > prev_strip.frame_start:
                 prev_strip.frame_end = int(overlap_end)
            else:
                 prev_strip.frame_end = int(strip_start)

        # === 3. CALCOLO SCALE (ANTI-SLIDING) ===
        scale_factor = 1.0
        ...
        # === 5. CREAZIONE NUOVA TRACCIA (SEMPRE SOPRA) ===
        # Creiamo una nuova traccia per ogni clip. 
        # Blender esegue le tracce dall'alto verso il basso (o viceversa in base alla view),
        # ma la logica è che la nuova deve "coprire" la vecchia.
        ...
        try:
            ...
            # === 6. BLEND SETUP (SOLO IN ENTRATA) ===
            strip.use_auto_blend = False
            strip.blend_out = 0
            
            if prev_strip:
                strip.blend_in = BLEND_FRAMES # Entra sfumando
                prev_strip.blend_out = 0      # Quella sotto rimane solida
            else:
                strip.blend_in = 0
    ...
    print(f"✅ Timeline completata ({track_counter} Tracce generate).")
