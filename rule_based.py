...
def determine_side(player_pos, ball_pos):
    # Blender X = NBA Y (Larghezza). 
    # Se Palla X > Player X -> Palla è a Destra
    p_x = player_pos[1] 
    b_x = ball_pos[1]
    return "dx" if b_x >= p_x else "sx"

# ==================== ANALISI FISICA (MANCANTE NEL CODICE PRECEDENTE) ====================
...
def determine_state_sequence(p_traj, b_traj, speeds, first_poss, shot_offset, shot_blender_start, shot_blender_end):
    """
    Determina lo STATO DI BASE (Celly, Catch, Dribble) frame per frame.
    ...
    for i in range(len(p_traj)):
        ...
        # Se siamo DOPO il tiro, forza Celly
        if current_blender_frame > shot_blender_end:
            states.append("celly_ice_vein")
            continue
            
        # Se non abbiamo ancora la palla -> Celly (Idle)
...
def setup_nla_tracks(armature, unique_anims, shot_anim_name, shot_blender_start_frame):
    ...
    Track 1 (Base): Contiene Celly, Catch, Dribble (in Loop/Repeat)
...
def main():
    print("="*50)
    print("🚀 AVVIO SCRIPT SYNC TIRO 360/25")
    print("="*50)
